...
export default function LoadFundsView({
  ...
  const [selectedCard, setSelectedCard] = useState<'voyager' | 'standard'>('voyager');
  ...
  const handleConfirm = (e: React.FormEvent) => {
    ...
    if (amount > savingsBalance) {
      setErrorMessage(`Insufficient savings balance. You only have $${savingsBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })} left in your Savings account.`);
      return;
    }
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-6 border-b border-outline-variant/20 mb-12">
        <div>
          <span className="text-primary font-bold tracking-wider text-sm font-headline uppercase">USD Account Portal</span>
          <h1 className="text-3xl lg:text-4xl font-extrabold font-headline mt-1">Load Funds to Elite Card</h1>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl p-8 border border-outline-variant/30 shadow-sm space-y-6">
          <form onSubmit={handleConfirm} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-outline uppercase font-headline">Source Asset Account</label>
              <div className="p-4 rounded-2xl bg-surface-container-low border border-outline-variant/20 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-on-surface text-sm font-headline">USD Global Savings</h4>
                    <p className="text-[11px] text-outline font-medium">Bole Financial Center Branch</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-xs text-outline font-medium uppercase font-headline">Available Balance</p>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-outline uppercase font-headline">Select Target Elite Card</label>
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-emerald-600">flight_takeoff</span>
                    <div>
                      <h4 className="font-bold text-on-surface text-sm font-headline">Elite Voyager Visa</h4>
                      <p className="text-[11px] text-outline font-medium">Ending in 8892</p>
                    </div>
                  </div>
                  <span className="material-symbols-outlined text-primary text-sm font-bold">
                    {selectedCard === 'voyager' ? 'radio_button_checked' : 'radio_button_unchecked'}
                  </span>
                </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-on-surface text-sm font-headline">Elite Standard Visa</h4>
                      <p className="text-[11px] text-outline font-medium">Ending in 4410</p>
                    </div>
                  </div>
                  <span className="material-symbols-outlined text-primary text-sm font-bold">
                    {selectedCard === 'standard' ? 'radio_button_checked' : 'radio_button_unchecked'}
                  </span>
                </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <label className="text-xs font-bold text-outline uppercase font-headline">Enter USD Amount to Load</label>
                <span className="text-xs font-semibold text-primary">Daily Limit: $50,000</span>
              </div>
              
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-2xl font-extrabold text-on-surface-variant font-headline">$</span>
                <input
                  type="number"
                  placeholder="0.00"
                  value={loadAmount}
                  ...
                  required
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm font-bold font-headline text-outline">USD</span>
              </div>
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div className="bg-slate-900 rounded-3xl p-8 text-white space-y-6 shadow-xl relative overflow-hidden">
            ...
            <h3 className="text-sm font-semibold font-headline uppercase text-white/50 relative z-10">Live Visa Balance</h3>

            {/* Simulated credit card face */}
            <div className={`p-6 rounded-2xl ${selectedCard === 'voyager' ? 'voyager-gradient' : 'elite-gradient'} h-44 flex flex-col justify-between border border-white/15 relative overflow-hidden shadow-2xl`}>
              <div className="flex justify-between items-start">
                <span className="font-headline font-bold text-sm tracking-wide">
                  {selectedCard === 'voyager' ? 'ELITE VOYAGER' : 'ELITE STANDARD'}
                </span>
                <span className="material-symbols-outlined text-white/85">contactless</span>
              </div>
              <div className="my-auto">
                <p className="text-[10px] text-white/50 uppercase tracking-widest mb-1 font-headline font-medium">Card Balance</p>
                <p className="font-mono text-2xl font-extrabold text-white">
                  ${(cardBalance + currentAmount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
              </div>
            ...
            <div className="divide-y divide-white/10 text-xs font-medium pt-2 relative z-10">
              <div className="flex justify-between py-2.5">
                ...
                <span className="font-bold text-emerald-400">Instant / Automatic</span>
              </div>
              <div className="flex justify-between py-2.5">
              ...
              <div className="flex justify-between py-2.5">
                <span className="text-white/60">Fly Rewards Earned (1 point per $5)</span>
                <span className="font-bold text-cyan-400">+{Math.floor(currentAmount / 5).toLocaleString()} ShebaMiles</span>
              </div>
              <div className="flex justify-between py-2.5 text-sm border-t border-white/20 pt-3">
                ...
                <span className="font-bold text-white font-mono">${currentAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 border border-outline-variant/30 rounded-2xl bg-surface-container-low text-xs leading-relaxed text-on-surface-variant flex gap-3">
            ...
            <p>
              Funding your visa card from global savings instantly credits your Visa network balance. This allows you to transact immediately globally with no processing delays of any kind.
            </p>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <div className="text-center space-y-2">
                <h3 className="text-2xl font-extrabold font-headline text-on-surface">Transfer Successful!</h3>
                <p className="text-sm text-on-surface-variant">Your card has been credited instantly.</p>
              </div>

              <div className="p-4 rounded-2xl bg-surface-container-low border border-slate-100 space-y-3.5 text-xs">
                <div className="flex justify-between">
                  <span className="text-outline uppercase font-semibold">Reference ID</span>
                  <span className="font-mono font-bold text-on-surface">{successReceipt.txId}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-outline uppercase font-semibold">Timestamp</span>
                  <span className="font-bold text-on-surface">{successReceipt.date}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-outline uppercase font-semibold">Loaded Amount</span>
                  <span className="font-bold text-on-surface font-mono">${successReceipt.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-outline uppercase font-semibold">Destination Wallet</span>
                  <span className="font-bold text-on-surface">Card ending in {selectedCard === 'voyager' ? '8892' : '4410'}</span>
                </div>
                <div className="flex justify-between border-t border-dashed border-outline-variant/30 pt-2.5">
                  <span className="font-semibold text-primary">Loyalty Rewards Issued</span>
                  <span className="font-bold text-emerald-600 font-headline">+{successReceipt.milesEarned.toLocaleString()} ShebaMiles</span>
                </div>
              </div>
